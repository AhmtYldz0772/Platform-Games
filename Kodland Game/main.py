...
for i in range(5):
    ...
    heart = Actor("heart",(x,y))
    heart.speed = random.randint(1,5)
    hearts.append(heart)
def heart_b():
    x = random.randint(0,600)
    y = random.randint(-150,-50)             "Bu kısım öğrenciler ile işlediğimiz kısım örneği ama biz yukarda class oluşturduk ve onun içinde nesnelerimiz yaptık"
    heart = Actor("heart",(x,y))
    heart.speed = random.randint(1,6)
    hearts.append(heart)
